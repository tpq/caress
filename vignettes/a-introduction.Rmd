...
Null layers are similar to orthogonal layers, except that they emit noise instead of an orthogonal signal. For categorical output, we can make a null layer by having it predict a dummy matrix. Below, a model is trained to have two channels, one that can predict the outcome and another that cannot. Together, they are able to reconstruct the input data.
...
inputs <- from_input(x_train)
split <- inputs %>%
  layer_to_stereo_and_mono(units = 8, name = "splitter")
outputs <- list(
  split$channel1 %>% to_output(y_train * 0 + (1/ncol(y_train)), name = "noY"),
  "yesY" = split$channel2 %>% to_output(y_train, name = "yesY"),
  "X" = split$channel12 %>% to_output(x_train, name = "X")
)
model <- prepare(inputs, outputs)
```

We train this model by providing a dummy matrix as one of the outputs.

```{r, results = "hide"}
history <- build(model,
                 x_train = x_train,
                 y_train = list("noY" = y_train * 0 + (1/ncol(y_train)),
                   "yesY" = y_train,
                   "X" = x_train),
